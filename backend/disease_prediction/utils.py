...
sym = pd.read_csv("disease_prediction\data\symptoms_df.csv")
precautions = pd.read_csv("disease_prediction\data\precautions_df.csv")
workout = pd.read_csv("disease_prediction\data\workout_df.csv")
description = pd.read_csv("disease_prediction\data\description.csv")
medications = pd.read_csv("disease_prediction\data\medications.csv")
diets = pd.read_csv("disease_prediction\data\diets.csv")
model = loadModel('disease_prediction\RandomForest.pkl')
...
class Preprocessor:
    def extractAsList(self, inp: str) -> list:
        patient_symptoms = [s.strip() for s in inp.split(',')]
        return patient_symptoms
    
    def __correct_spelling(self, symptom: list) -> list:
        closest_match, score = process.extractOne(symptom, symptoms_list_processed.keys())
        return closest_match if score >= 80 else None

    def spellingCorrector(self, symptoms: list) -> list:
        patient_symptoms = []
        for symptom in symptoms:
            match = self.__correct_spelling(symptom)
            if match:
                patient_symptoms.append(match)
        return patient_symptoms
...
class DiseasePredictionOrchestrator:
    def __init__(self, symptomsStr: str):
        self.symptomsStr = symptomsStr
    
    def run(self) -> dict:
        if not self.symptomsStr or not self.symptomsStr.strip():
            return {
                "status": "error",
        ...
        if not sym_list:
            return {
                "status": "error",
                "message": "No valid symptoms detected after processing.",
                "data": None
            }
        ...
        if pred in ["Unknown Disease", "Prediction Error"]:
            return {
                "status": "error",
                "message": "Unable to predict disease based on given symptoms.",
                "data": None
            }


        info = InfoExtractor(pred)

        output = {
            "disease": pred,
            "desc": info.extractDesc(),
            "diet": info.extractDiet(),
            "med": info.extractMed(),
            "prec": info.extractPrecaution(),
            "workout": info.extractWorkout()
        }
